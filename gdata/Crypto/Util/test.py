...
def print_timing(size, delta, verbose):
    if verbose:
        if delta == 0:
            print
            'Unable to measure time -- elapsed time too small'
        else:
            print
            '%.2f K/sec' % (size / delta)
        ...
        if (ciph != None):
            ...
            if 0:
                ...
                if a != "\xEE\xA9\xD0\xA2\x49\xFD\x3B\xA6\xB3\x43\x6F\xB8\x9D\x6D\xCA\x92":
                    if verbose: print
                    'CAST test failed: value of "a" doesn\'t match'
                if b != "\xB2\xC9\x5E\xB0\x0C\x31\xAD\x71\x80\xAC\x05\xB8\xE8\x3D\x69\x6E":
                    if verbose: print
                    'CAST test failed: value of "b" doesn\'t match'
