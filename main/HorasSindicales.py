...
class HorasSindicales(BaseHandler):
    ...
    def execute(self):
        ...
        http = decorator.http()
        try:
            events = service.events().list(calendarId='8bsm4ma7nh11fd348p31l6fdu8@group.calendar.google.com', timeMin=start_min, timeMax=start_max).execute(http)
        except HttpError:
            events = ""
        if events != "":
            seguir = True
            while seguir:
                ...
                if page_token:
                    events = service.events().list(calendarId='8bsm4ma7nh11fd348p31l6fdu8@group.calendar.google.com', timeMin=start_min, timeMax=start_max, pageToken=page_token).execute(http)
                else:
                    seguir = False
