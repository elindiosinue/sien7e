...
def urlPlatilla(fichPy, tipo):
    sep = "/"
    if str.find(fichPy,"\\") != -1:
        sep = "\\"
    arr = str.split(fichPy, sep)
    fich = str.split(arr[arr.__len__() - 1], ".")
    directorio = os.path.dirname(fichPy) + sep + ".." + sep + "plantillas"
    if tipo == 2:
         directorio = directorio + sep + "include"
    path = os.path.join(directorio, fich[0].lower() + ".html")
